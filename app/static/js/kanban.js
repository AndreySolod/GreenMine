var translations = function(str, transl_dict) {
    if(str in transl_dict) {
        return transl_dict[str]
    }
    return str
}
...
var materialKanban = (function () {
    ...
    return {
        ...
        initialize: function (
            parentID, udConfigJSON, websocket_element, escapeRequired, enableDragnDrop, enableDragnDropGroups, translation_dict) {
            var stdConfigJSON = {                      // dataJSON - готовый ajax-запрос, к которому применяется метод done()
                "refresh": 0,                          // udConfigJSON - это уже готовый стандартный конфиг, который передаётся в виде json-а
                "staticColumns": [{                    // changeElementEndpoint - это словарь, содержащий все параметры для выполнения ajax-запроса на изменение статуса задачи
                    "COLUMN_ID": "1",                  // changeGroupEndpoint - это словарь, содержащий все параметры для выполнения ajax-запроса на изменение порядка группы
                    "COLUMN_TITLE": "To Do",
            ...
            if (parent.length > 0) {
                ...
                /* bind websockets event 'refresh' */

                websocket_element.on('refresh', function(data) {
                    if (container.children('span').length == 0) {
                        ...
                        if(data.get_data) {
                            ...
                            // Далее выполнить через QuerySelectorAll и изменить только конкретную задачу
                            let _el = document.querySelectorAll(`[itemid="${data.task_id}"]`)
                            var dropData = {
                ...
                console.error("Can't find parentID: " + parentID);
            }
             ...
             ***********************************************************************/
            function getData() {
                websocket_element.emit('get data', null, data => {
            ...
            function drawKanbanRegion(itemsData) {
                ...
                if (columnsData && columnsData.length > 0) {
                    ...
                    if (itemsData.row && itemsData.row.length > 0) {
                        ...
                        util.noDataMessage.show(cardContainer, translations("No data for displaying", translation_dict));
                    }
            ...
            function addDragulaToItems() {
                ...
                drakeItems = dragula($(parent.find('.kb-item-container')).toArray(), {
                    ...
                    accepts: function (el, target, source, sibling) {
                        ...
                        if (configJSON.allowDragItemsBetweenGroups) { // TODO Здесь проверяется, что возможно перемещение из одного контейнера в другой. Возможно, потом сюда прикрутить кастомную проверку
                            return true;
                        }
                ...
                */
                ...
                drakeItems.on("drop", function (el, target, source, sibling) {
                    ...
                    websocket_element.emit('change element', dropData, data => {
                        if (data.status !== 'success'){
                            ...
                            if (data.hasOwnProperty('message')) {
                                alert(translations(data.message, translation_dict))
                            }
            ...
            function addDragulaToGroups() {
                ...
                */
                ...
                drakeGroups.on("drop", function (el, target, source, sibling) {
                    ...
                    websocket_element.emit('change group', dropData, data => {
                        if(data.status !== "success") {
                            if (sourceItemSibling) {
                                _el.insertBefore(sourceItemSibling);
                            } else {
