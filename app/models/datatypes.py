...
CreatedAt = Annotated[datetime.datetime, so.mapped_column(sa.DateTime(timezone=True), index=True, default=utcnow, info={'label': _l('Created at')})]
UpdatedAt = Annotated[Optional[datetime.datetime], so.mapped_column(sa.DateTime(timezone=True), info={'label': _l('Updated at')})]
