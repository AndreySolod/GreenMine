...
class Host(HasComment, db.Model, HasHistory):
    ...
    operation_system_family_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey(OperationSystemFamily.id, ondelete='SET NULL'), index=True, info={'label': _l("Operation system family")})
    ...
    from_network_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey('network.id', ondelete='CASCADE'), index=True, info={"label": _l("Refers to Network")})
    ...
    device_type_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey(DeviceType.id, ondelete='SET NULL'), index=True, info={'label': _l("Device type")})
    device_type: so.Mapped[DeviceType] = so.relationship(info={'label': _l("Device type")})
    device_vendor_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey(DeviceVendor.id, ondelete='SET NULL'), index=True, info={'label': _l("Device vendor")})
    device_vendor: so.Mapped[DeviceVendor] = so.relationship(info={'label': _l("Device vendor")})
    device_model_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey(DeviceModel.id, ondelete='SET NULL'), index=True, info={'label': _l("Device model")})
...
class DefaultPortAndTransportProto(db.Model):
    ...
    access_protocol_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey('access_protocol.id', ondelete='CASCADE'), index=True, info={'label': _l("Application Layer Protocol")})
...
class Service(HasComment, db.Model, HasHistory):
    ...
    updated_by: so.Mapped["User"] = so.relationship(lazy='select', foreign_keys=[updated_by_id], info={'label': _l("Updated by")}) # type: ignore
    port: so.Mapped[int] = so.mapped_column(info={'label': _l("Port")})
    access_protocol_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey(AccessProtocol.id, ondelete='SET NULL'), index=True, info={'label': _l("Application Layer Protocol")})
    access_protocol: so.Mapped["AccessProtocol"] = so.relationship(lazy='joined', info={'label': _l("Application Layer Protocol")})
    port_state_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey(ServicePortState.id, ondelete='SET NULL'), index=True, info={'label': _l("Port status")})
    ...
    transport_level_protocol_id: so.Mapped[Optional[int]] = so.mapped_column(sa.ForeignKey(ServiceTransportLevelProtocol.id, ondelete='SET NULL'), index=True, info={'label': _l("Transport Layer Protocol")})
    transport_level_protocol: so.Mapped["ServiceTransportLevelProtocol"] = so.relationship(lazy='joined', info={'label': _l("Transport Layer Protocol")})
    host_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(Host.id, ondelete='CASCADE'), index=True, info={'label': _l("Host")})
