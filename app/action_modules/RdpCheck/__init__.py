...
def console_read(console_data):
    ...
    if "Detected RDP on" in console_data['data']: #Detected RDP on 10.0.0.1:3389     (name:SOME-NAME) (domain:SOME-DOMAIN) (domain_fqdn:SOME-FQDN) (server_fqdn:SOME-FQDN) (os_version:10.0.19041) (Requires NLA: Yes)
        rdp_props = re.findall(r".*?- Detected RDP on .*?\(name:(.*?)\) \(domain:(.*?)\) \(domain_fqdn:(.*?)\) \((server_fqdn:(.*?)\) \(os_version:(.*?)\) \(Requires NLA: (.*?)\)", console_data['data'])
        RDP_PROPS = rdp_props[0]


def action_run(target: models.Service, running_user_id: int, session: Session) -> Dict[str, str]:
    ...
    if RDP_PROPS[5] == "No":
        ...
        if not issue:
            issue_template = session.scalars(sa.select(models.IssueTemplate).where(models.IssueTemplate.by_slug == 'rdp_without_nla')).first()
...
def exploit(filled_form: dict, running_user_id: int, default_options: dict, locale: str, project_id: int) -> None:
    ...
    with so.sessionmaker(db.engine, autoflush=False)() as session:
        for target in filled_form['targets']:
            ...
            action_run(target, running_user_id, session)
        session.commit()
