...
# Rebuilding Flask-Momen from miguelgrindberg (https://github.com/miguelgrinberg/Flask-Moment/blob/main/src/flask_moment/__init__.py) for Content-Security Policy edit
...
class moment(object):
    ...
    def include_moment(cls, version=default_moment_version, local_js=None,
                       no_js=None, sri=None, with_locales=True, locale="en",
                       csp_nonce: str=''):
        ...
        :param with_locales: If ``True``, include the version of moment.js that
                             has all the locales.
        :param locale: The default locale for moment. It will be render on page
        in default script
        :param csp_nonce: string CSP Nonce, that use to 'sign' script tags as
        'known' script and to prevent an XSS attack.
        """
        mjs = ''
        ...
        if not no_js:
            if local_js is not None:
                if not sri:
                    mjs = f'<script src="{local_js}" nonce="{csp_nonce}"></script>\n'
                else:
                    mjs = (f'<script src="{local_js}" integrity="{sri}" nonce="{csp_nonce}" '
                ...
                if not sri:
                    mjs = (f'<script nonce="{csp_nonce}" src="https://cdnjs.cloudflare.com/ajax/'
                           f'libs/moment.js/{version}/{js_filename}"></script>\n')
                else:
                    mjs = ('<script src="https://cdnjs.cloudflare.com/ajax/'
                           f'libs/moment.js/{version}/{js_filename}" integrity="{sri}" '
                           f'crossorigin="anonymous" nonce="{csp_nonce}"></script>\n')
        return Markup(f'{mjs}\n<script nonce="{csp_nonce}">\n{cls.flask_moment_js(locale)}\n</script>\n''')

    @staticmethod
    def locale(language='en', auto_detect=False, customization=None, csp_nonce:str=''):
        ...
        :param csp_nonce: a CSP Nonce string, that used to prevent an XSS attack.
        """
        if auto_detect:
        ...
        if customization:
            return Markup(
                f'<script nonce="{csp_nonce}">\nmoment.locale("{language}", {customization});\n</script>')
        return Markup(
            '<script>\nmoment.locale("{}");\n</script>'.format(language))

    @staticmethod
    def flask_moment_js(locale="en"):
        ...
        return '''moment.locale("{}");{}\n{}'''.format(locale, default_format, js_code)
...
class Moment(object):
    ...
    def flask_moment_js(self, locale="en"):
        return current_app.extensions['moment'].flask_moment_js(locale)
